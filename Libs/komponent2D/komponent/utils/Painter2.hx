...
class Painter2
{
	...
	public static inline function drawImage5(image:Image, x:Float, y:Float, rotation:Float, centerX:Float, centerY:Float, flipX:Bool = false, flipY:Bool = false, sourceRect:Rectangle = null, tiledWidth:Int = 0, tiledHeight:Int = 0, fillScreen:Bool = false):Void
	{
		...
		if (sourceRect == null)
		{
			dw = image.width * Painter2.scaleX;
			dh = image.height * Painter2.scaleY;
		...
		{
			dw = sourceRect.width * Painter2.scaleX;
			dh = sourceRect.height * Painter2.scaleY;
		...
		if (tiledWidth > 0 && tiledHeight > 0 || fillScreen)
		{
			...
			while (ty < tiledHeight)
			{
				while (tx < tiledWidth)
				{
					//Painter2.drawImage2(image, sx, sy, sw, sh, x + tx, y + ty, dw, dh, rotation, centerX, centerY);
					// TODO: rotation
	...
	public static inline function drawRect2(x:Float, y:Float, width:Float, height:Float, angle:Float = 0, centerX:Float = null, centerY:Float = null, strength:Float = 1.0):Void
	{
		...
		width *= camera.fullScaleX * Painter2.scaleX;
		height *= camera.fullScaleY * Painter2.scaleY;
	...
	{
		width *= camera.scaleX * Painter2.scaleX;
		height *= camera.scaleY * Painter2.scaleY;
		// use average scale
		strength *= (camera.fullScaleX + camera.fullScaleY) / 2;
		
		x = (x - Painter2.camera.x - Screen.halfWidth) * camera.fullScaleX + Screen.halfWidth;
		y = (y - Painter2.camera.y - Screen.halfHeight) * camera.fullScaleY + Screen.halfHeight;
	...
	public static inline function drawString(text: String, x: Float, y: Float, scaleCenterX: Float = 0.0, scaleCenterY: Float = 0.0)
	{
		x = (x - Screen.halfWidth) * camera.fullScaleX + Screen.halfWidth;
		y = (y - Screen.halfHeight) * camera.fullScaleY + Screen.halfHeight;
		//g2.drawString(text, x, y, Painter2.scaleX * camera.fullScaleX, Painter2.scaleY * camera.fullScaleY, scaleCenterX, scaleCenterY);
	...
	{
		x1 = (x1 - Screen.halfWidth) * Painter2.scaleX + Screen.halfWidth;
		y1 = (y1 - Screen.halfHeight) * Painter2.scaleY + Screen.halfHeight;
		
		x2 = (x2 - Screen.halfWidth) * Painter2.scaleX + Screen.halfWidth;
		y2 = (y2 - Screen.halfHeight) * Painter2.scaleY + Screen.halfHeight;
	 ...
	 */
	public static function drawPolygon(x:Float, y:Float, vertices:Array<Vector2>, strength:Float = 1)
	{
		...
		while (iterator.hasNext())
		{
			...
			Painter2.drawLine2(v1.x + x, v1.y + y, v2.x + x, v2.y + y, strength);
			v1 = v2;
		}
		Painter2.drawLine2(v1.x + x, v1.y + y, v0.x + x, v0.y + y, strength);
	}
	
	public static inline function set(color:Color, alpha:Float, font:Font = null):Void
	{
		Painter2.color = color;
		Painter2.alpha = alpha;
		Painter2.setScale(scaleX, scaleY);
		Painter2.font = font;
	}
	
	public static inline function setScale(scaleX:Float, scaleY:Float)
	{
		Painter2.scaleX = scaleX;
		Painter2.scaleY = scaleY;
	}
	
	private static inline function set_backbuffer(value:Image):Image
	{
		g2 = value.g2;
		g2.pushTransformation(Matrix.identity());
		g4 = value.g4;
		return value;
	}
